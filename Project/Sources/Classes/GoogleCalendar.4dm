...
/*
    ...
    Google Calendar for changes and dispatches callbacks when events are created, deleted, or modified.
    
    Two modes:
    - Push (webhook): If endPoint is provided, creates a Google Calendar watch channel
      that receives real-time notifications via webhook.
    - Pull: If no endPoint, polls the Calendar events API with sync tokens
      at a configurable interval to detect changes.
    
    Parameters:
        $inParameters.onCreate : 4D.Function - Called when an event is created. Receives {eventType; IDs}.
        $inParameters.onDelete : 4D.Function - Called when an event is deleted. Receives {eventType; IDs}.
        $inParameters.onModify : 4D.Function - Called when an event is modified. Receives {eventType; IDs}.
        $inParameters.endPoint : Text - Optional. Webhook URL for push mode. If omitted, uses pull mode.
        $inParameters.timer : Integer - Optional. Polling interval in seconds for pull mode (default: 30).
        $inCalendarId : Text - Optional. Calendar ID to watch. If omitted, watches the default calendar.
